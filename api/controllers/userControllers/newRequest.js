...
const newRequest = async (req,res,next)=>{
    ...
    try{
        ...
        let pendingRequestsSent = req.body.mentees? await user.getPendingRequestsSentToMentees() : await user.getPendingRequestSentToMentor()
        ...
        if(req.body.mentees && user.role.includes("mentor") && (user.mentees.length + ments.length > 5)) return res.status(400).json({message: `You already have ${user.mentees.length} mentees, you can only send requests to ${5 - user.mentees.length} more.`})
        if(req.body.mentor && (pendingRequestsSent.length == 1)) return res.status(400).json({message: "You already sent a request to a mentor. Cancel it to send a new one."})
        if(!user.role.includes("mentor") && req.body.mentees) return res.status(400).json({message: "Ups! You can't add mentees to a mentee."})
        ...
        if(mentsIds.length){
            ...
            foundMents.forEach(ment => {
                const message = normalizedMents.find(m => m._id == ment._id.toString()).message
                requests.push({
                    from: user,
                    to: ment,
                    fromRole: requestedRole,
                    message: message.length? message : `¡Hola! Me gustaría ser tu ${requestedRole}` 
                })
            })
            //Create requests
